...
rabin_ids.Configuration = function (shares, quorum) {
  ...
  this.encode = function (secret) {
    'use strict';
    const chunks = Math.ceil(secret.length / this.quorum);
    const shs = [];
    for (let k = 0; k < this.shares; k++) {shs[k] = {data: [], degree: k + 1, original_length: secret.length};}
    let i = 0;

    for (let chunk = 0; chunk < chunks; chunk++) {
      const coeffs = [];
      for (let j = 0; j < this.quorum; j++) {
